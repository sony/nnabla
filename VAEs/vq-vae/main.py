...
from models.vq_vae_pixel_cnn import GatedPixelCNN 
...
def make_parser():
	...
	parser.add_argument('--pixelcnn', action='store_true')
	return parser
...
def train(data_iterator, monitor, config, comm, args):
	...
	if args.pixelcnn:
		base_model = model
		pixelcnn_model = GatedPixelCNN(config)
		nn.load_parameters(config['pixelcnn']['base_model_checkpoint'])
		trainer = TrainerPrior(base_model, pixelcnn_model, solver, train_loader, monitor_loss, 
		config, comm)
	else:
		trainer = VQVAEtrainer(model, solver, train_loader, monitor_loss, monitor_recon, config, comm)
	...
	for epoch in range(config['train']['num_epochs']):
		iteration = trainer.train(iteration)

		if comm.rank == 0:
			if epoch%config['train']['save_param_step_interval'] == 0 or epoch == config['train']['num_epochs']-1:
				trainer.save_checkpoint(config['model']['saved_models_dir'], epoch)
