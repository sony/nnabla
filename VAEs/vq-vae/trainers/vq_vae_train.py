...
class VQVAEtrainer(object):
    ...
    def __init__(self, model, solver, data_loader, logger, logger_recon, config, comm):
        ...
        self.iterations_per_epoch = int(
            np.ceil(config['dataset']['train_size']/config['train']['batch_size']))
    ...
    def scale_back_var(self, img_var):
        img = img_var.d + 0.5
        img = (img - img.min(axis=(2, 3)).reshape((img.shape[0:2]+(1, 1))))/img.max(
            axis=(2, 3)).reshape((img.shape[0:2]+(1, 1)))
        return img
    ...
    def update(self, iteration):
        ...
        for i in range(self.iterations_per_epoch):
            ...
            # import pdb; pdb.set_trace()
            loss, recon_loss, perplexity, img_recon = self.compute_loss(
                img_var, iteration)
            # try:
