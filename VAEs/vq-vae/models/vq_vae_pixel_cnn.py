...
class VectorQuantizer(object):
    ...
    def __call__(self, x):
        ...
        quantized = F.batch_matmul(
            encodings, self.embedding_weight).reshape(x.shape)
...
class VectorQuantizerEMA(object):
    ...
    def __init__(self, embedding_dim, num_embedding, commitment_cost, rng,
        ...
        self.embedding_weight = nn.Variable.from_numpy_array(
            np.random.randn(self.num_embedding, self.embedding_dim))
        self.embedding_weight.need_grad = True
        self.ema_cluster_size = nn.Variable.from_numpy_array(
            np.zeros(self.num_embedding,))
        self.ema_cluster_size.need_grad = False
        self.ema_w = nn.Variable.from_numpy_array(
            np.random.randn(self.num_embedding, self.embedding_dim))
    ...
    def __call__(self, x, iteration, is_training=True):
        ...
        try:
            ...
            emb_wt_squared = F.transpose(
                F.sum(self.embedding_weight**2, axis=1, keepdims=True), (1, 0))
            distances = x_flat_squared + emb_wt_squared - 2 * \
                F.batch_matmul(x_flat, F.transpose(
        ...
        try:
            if is_training:
                self.ema_cluster_size = self.decay*self.ema_cluster_size + \
                    (1-self.decay)*F.sum(encodings, axis=0)
                ...
                self.embedding_weight = self.ema_w / \
                    self.ema_cluster_size.reshape(
                        self.ema_cluster_size.shape[:1]+(1,))
            import pdb
            pdb.set_trace()
        except:
            import pdb
