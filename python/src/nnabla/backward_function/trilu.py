...
def get_trilu_mask(shape, k, upper):
    mask_data = np.ones(shape)
    if upper:
        mask_data = np.triu(mask_data, k)
    else:
        mask_data = np.tril(mask_data, k)
    return nn.Variable.from_numpy_array(mask_data)


def trilu_backward(grad_inputs, inputs, input_shapes, outputs, output_shapes, k=0, upper=True):
    """
    Args:
    ...
    # for more detail.
    dy = grad_inputs[0]
    mask = get_trilu_mask(dy.shape, k, upper)
    mask = no_grad(mask)
    dx0 = F.mul2(mask, dy)
    return dx0
