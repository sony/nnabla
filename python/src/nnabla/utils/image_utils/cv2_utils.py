...
def _cvtColor_helper(img, num_channels):
    ...
    if len(img.shape) == 2:
        return convert_channel_from_gray(img, num_channels)

    elif img.shape[-1] == 3:
        return convert_channel_from_bgr(img, num_channels)

    elif img.shape[-1] == 4:
        return convert_channel_from_bgra(img, num_channels)
...
def imresize(img, size, interpolate="bilinear", channel_first=False):
    ...
    resized = cv2.resize(
        img, size, interpolation=interpolations_map[interpolate])
