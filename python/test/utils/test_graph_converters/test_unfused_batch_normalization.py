...
@pytest.mark.parametrize('graph_ref, graph_act', [(resnet_ref, small_fbn_resnet)])
def test_unfused_batch_normalization(seed, test, graph_ref, graph_act):
    ...
    # FunctionModifier
    modifiers = []
    modifiers.append(GC.UnfusedBatchNormalizationModifier())

    y_act = GC.GraphConverter(modifiers).convert(y_tgt)

    # Ref Graph
    y_ref = graph_ref(x, test=test, name='bn-graph-ref')
