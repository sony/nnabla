...
@pytest.mark.parametrize("seed", [313])
@pytest.mark.parametrize("alpha", [0, 0.1, -0.1])
def test_leaky_relu_forward_backward_with_reset(seed, alpha, ctx, func_name):
    from nbla_test_utils import cap_ignore_region, function_tester
    rng = np.random.RandomState(seed)
    inputs = [
        cap_ignore_region(
    ...
    reset_inputs = [
        cap_ignore_region(
            rng.randn(1, 2, 4).astype(np.float32) * 2,
            (-1e-3, 1e-3))]
    function_tester(rng, F.leaky_relu, ref_leaky_relu, inputs,
                    func_args=[alpha], ctx=ctx, func_name=func_name, reset_inputs=reset_inputs)
