...
@pytest.mark.parametrize("fname, ctx, func_name",
                         list_ctx_and_func_name(['logical_and_scalar', 'logical_or_scalar', 'logical_xor_scalar']))
@pytest.mark.parametrize("val", [False, True])
def test_logical_scalar_forward_backward_with_reset(val, seed, fname, ctx, func_name):
    func = getattr(F, fname)
    ref_func = getattr(np, fname.replace('_scalar', ''))
    rng = np.random.RandomState(seed)
    inputs = [rng.randint(0, 2, size=(2, 3, 4)).astype(np.float32)]
    reset_inputs = [rng.randint(0, 2, size=(3, 2, 4)).astype(np.float32)]
    function_tester(rng, func, ref_func, inputs, [val],
                    ctx=ctx, backward=[False], func_name=func_name, reset_inputs=reset_inputs)
...
@pytest.mark.parametrize("fname, ctx, func_name",
                         list_ctx_and_func_name(['greater_scalar',
                                                 ...
                                                 'not_equal_scalar']))
@pytest.mark.parametrize("val", [-0.5, 0., 1.])
def test_logical_scalar_compare_forward_backward_with_reset(val, seed, fname, ctx, func_name):
    opstr = opstrs[fname.replace('_scalar', '')]
    func = getattr(F, fname)
    ref_func = eval('lambda x, y: x {} y'.format(opstr))
    rng = np.random.RandomState(seed)
    ...
    reset_inputs = [rng.randint(0, 2, size=(3, 2, 4)).astype(np.float32)
                    for _ in range(1)]
    reset_inputs[0][..., :2] = val
    function_tester(rng, func, ref_func, inputs, [val],
                    ctx=ctx, backward=[False, False], func_name=func_name, reset_inputs=reset_inputs)
...
@pytest.mark.parametrize("fname, ctx, func_name",
                         list_ctx_and_func_name(['logical_and', 'logical_or', 'logical_xor']))
def test_logical_binary_forward_backward_with_reset(seed, fname, ctx, func_name):
    func = getattr(F, fname)
    ref_func = getattr(np, fname)
    rng = np.random.RandomState(seed)
    inputs = [rng.randint(0, 2, size=(2, 3, 4)).astype(np.float32)
...
@pytest.mark.parametrize("fname, ctx, func_name",
                         list_ctx_and_func_name(['greater',
                                                 ...
                                                 'equal',
                                                 'not_equal']))
def test_logical_binary_compare_forward_backward_with_reset(seed, fname, ctx, func_name):
    func = getattr(F, fname)
    opstr = opstrs[fname]
    ref_func = eval('lambda x, y: x {} y'.format(opstr))
    rng = np.random.RandomState(seed)
    ...
    reset_inputs = [rng.randint(0, 2, size=(3, 2, 4)).astype(np.float32)
                    for _ in range(2)]
    reset_inputs[0][..., :2] = reset_inputs[1][..., :2]
    function_tester(rng, func, ref_func, inputs,
                    ctx=ctx, backward=[False, False], func_name=func_name, reset_inputs=reset_inputs)
# ----------------------------------------------------------------------------
# Logical not
...
@pytest.mark.parametrize("seed", [314])
@pytest.mark.parametrize("fname, ctx, func_name", list_ctx_and_func_name(['logical_not']))
def test_logical_not_forward_backward_with_reset(seed, fname, ctx, func_name):
    func = getattr(F, fname)
    ref_func = getattr(np, fname)
    rng = np.random.RandomState(seed)
    inputs = [rng.randint(0, 2, size=(2, 3, 4)).astype(np.float32)]
