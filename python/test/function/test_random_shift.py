...
@pytest.mark.parametrize("ctx, func_name", ctxs)
@pytest.mark.parametrize("seed", [313])
@pytest.mark.parametrize("inshape,reset_inshape", [((7, 8, 9), (7, 9, 8))])
@pytest.mark.parametrize("shifts", [None, (0, 0, 2,), (0, 2, 0), (0, 2, 2)])
@pytest.mark.parametrize("border_mode", ["nearest", "reflect", "constant"])
@pytest.mark.parametrize("constant_value", [0, -100])
def test_random_shift_forward_backward_with_reset(seed, inshape, reset_inshape, shifts, border_mode, constant_value,
                                                  ctx, func_name):
    rng = np.random.RandomState(seed)
    inputs = [rng.randn(*inshape).astype(np.float32)]
    i = nn.Variable(inputs[0].shape, need_grad=True)
    ...
    o.forward()
    # reset input
    reset_input = [rng.randn(*reset_inshape).astype(np.float32)]
    i.reset_shape(reset_input[0].shape, True)
    i.d = reset_input[0]
    o.forward()

    result_shifts = (0, 0, 0)
    max_correl = 0
    for shift_amount in itertools.product(
            *map(tuple, map(lambda x: range(*x), [(-2, 3) for _ in range(len(reset_inshape))]))):
        r = scipy_shift(reset_input[0], shift_amount,
                        mode=border_mode, cval=constant_value)
        correl_and_p = pearsonr(o.d.flatten(), r.flatten())
        if correl_and_p[0] > max_correl:
            result_shifts = shift_amount
            max_correl = correl_and_p[0]
    ref = scipy_shift(reset_input[0], result_shifts,
                      mode=border_mode, cval=constant_value)
    if shifts is None:
        shifts = (0,) * len(reset_input[0].shape)
    for result, shift_range in zip(result_shifts, shifts):
        assert abs(result) <= shift_range
