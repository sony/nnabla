...
@pytest.mark.parametrize("batch_size,reset_batch_size", [(3, 2)])
@pytest.mark.parametrize("input_size,reset_input_size", [(2, 3)])
@pytest.mark.parametrize("hidden_size,reset_hidden_size", [(3, 2)])
@pytest.mark.parametrize("with_bias", [True, False])
def test_rnn_with_reset(seed, num_layers, nonlinearity, dropout, bidirectional, training, seq_len, reset_seq_len,
                        batch_size, reset_batch_size, input_size, reset_input_size, hidden_size, reset_hidden_size,
                        with_bias, ctx, func_name):
    from nbla_test_utils import function_tester
    ...
    reset_inputs += [rng.randn(num_directions, reset_hidden_size,
                               reset_input_size + reset_hidden_size)]
    if num_layers > 1:
        inputs += [rng.randn(max(1, num_layers - 1), num_directions, hidden_size,
                             num_directions * hidden_size + hidden_size).astype(np.float32)]
        reset_inputs += [rng.randn(max(1, num_layers - 1), num_directions, reset_hidden_size,
    ...
    if with_bias:
        inputs += [rng.randn(num_layers, num_directions,
                             hidden_size).astype(np.float32)]
        reset_inputs += [rng.randn(num_layers, num_directions,
                                   reset_hidden_size).astype(np.float32)]
    else:
        inputs += [None]
        reset_inputs += [None]
    ...
    function_tester(rng, F.rnn, execute_fixed_length_rnn, inputs, func_kwargs=dict(
        num_layers=num_layers, nonlinearity=nonlinearity, dropout=dropout, bidirectional=bidirectional,
        training=training), atol_f=2e-1, atol_b=2e-1, dstep=1e-3, backward=backward, ctx=ctx, func_name=func_name,
                    ref_grad=get_rnn_grad, disable_half_test=True, reset_inputs=reset_inputs)
