...
@pytest.mark.parametrize("bias", [True, False])
@pytest.mark.parametrize("quantize_zero_to", [0.0, -1.0, 1.0])
def test_binary_weight_affine_forward_backward_with_reset(seed, base_axis, weight_shape, reset_weight_shape, bias,
                                                          quantize_zero_to,
                                                          ctx, func_name):
    from nbla_test_utils import function_tester
    rng = np.random.RandomState(seed)
    ...
    # Alpha: Scaling factor
    inputs += [np.zeros(np.prod(weight_shape[1:])).astype(np.float32)]
    reset_inputs += [np.zeros(np.prod(reset_weight_shape[1:])
                              ).astype(np.float32)]
    # Bias
    if bias:
        inputs += [rng.randn(*weight_shape[1:]).astype(np.float32)]
    ...
    function_tester(rng, F.binary_weight_affine, ref_binary_weight_affine, inputs,
                    func_args=[base_axis, quantize_zero_to],
                    atol_b=1e-2, backward=[True, True, False, False, True], ctx=ctx, func_name=func_name,
                    ref_grad=ref_grad_binary_weight_affine, reset_inputs=reset_inputs)
