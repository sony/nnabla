...
@pytest.mark.parametrize("ctx, func_name", ctxs)
@pytest.mark.parametrize("seed", [313])
def test_hard_sigmoid_forward_backward_with_reset(seed, ctx, func_name):
    from nbla_test_utils import function_tester
    rng = np.random.RandomState(seed)
    inputs = [
        np.clip(np.abs(rng.randn(2, 3, 4).astype(np.float32)) * 1e4, 1e-2, 1e4)]
    ...
    function_tester(rng, F.hard_sigmoid, ref_hard_sigmoid, inputs,
                    ctx=ctx, func_name=func_name, ref_grad=ref_hard_sigmoid_backward, reset_inputs=reset_inputs)
