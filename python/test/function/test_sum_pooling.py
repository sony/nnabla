...
def test_sum_pooling_2d(seed, inshape, kernel, stride, pad, ignore_border,
                        ctx, func_name):
    from nbla_test_utils import function_tester
    rng = np.random.RandomState(seed)
    inputs = [rng.randn(*inshape).astype(np.float32)]
    func_args = [kernel, stride, ignore_border, pad]
    function_tester(rng, F.sum_pooling, ref_sum_pooling_2d, inputs=inputs,
...
def test_sum_pooling_3d(seed, inshape, kernel, stride, pad, ignore_border,
                        ctx, func_name):
    from nbla_test_utils import function_tester
    rng = np.random.RandomState(seed)
    inputs = [rng.randn(*inshape).astype(np.float32)]
    func_args = [kernel, stride, ignore_border, pad]
    function_tester(rng, F.sum_pooling, ref_sum_pooling_3d, inputs=inputs,
                    func_args=func_args, func_name=func_name, ctx=ctx,
                    atol_f=1e-6, atol_b=1e-2)
