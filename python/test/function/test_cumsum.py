...
@pytest.mark.parametrize("reverse", [True, False])
@pytest.mark.parametrize("ctx, func_name", list_context('CumSum'))
def test_cumsum_forward_backward_with_reset(seed, axis, exclusive, reverse, ctx, func_name):
    from nbla_test_utils import function_tester
    rng = np.random.RandomState(seed)
    inputs = [(rng.randn(5, 3, 4)).astype(np.float32)]
    reset_inputs = [(rng.randn(1, 3, 4)).astype(np.float32)]
    function_tester(rng, F.cumsum, ref_cumsum, inputs, func_args=[axis, exclusive, reverse],
                    ctx=ctx, func_name=func_name, atol_b=3e-3, disable_half_test=True, reset_inputs=reset_inputs)
