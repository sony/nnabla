...
def test_tanh_double_backward(seed, ctx, func_name):
    ...
    inputs = [
        cap_ignore_region(
            ...
            (-1e-3, 1e-3))]
    backward_function_tester(rng, F.tanh,
                             inputs=inputs,
...
@pytest.mark.parametrize("ctx, func_name", ctxs)
@pytest.mark.parametrize("seed", [313])
def test_tanh_forward_backward_with_reset(seed, ctx, func_name):
    from nbla_test_utils import function_tester
    rng = np.random.RandomState(seed)
    inputs = [rng.randn(2, 3, 4).astype(np.float32) * 1]
    reset_inputs = [rng.randn(3, 2, 3).astype(np.float32) * 1]
    function_tester(rng, F.tanh, np.tanh, inputs, ctx=ctx,
