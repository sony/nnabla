...
@pytest.mark.parametrize("ctx, func_name", ctxs)
@pytest.mark.parametrize("seed", [313])
@pytest.mark.parametrize("inshape,reset_inshape", [((1, 2, 3), (2, 4, 6)), ((2, 2, 4, 6), (2, 2, 2, 4, 6))])
@pytest.mark.parametrize("kernel", [(1, 1), (2, 3), (2, 1, 2)])
@pytest.mark.parametrize("channel_last", [False, True])
def test_unpooling_forward_backward_with_reset(seed, inshape, reset_inshape, channel_last, kernel, ctx, func_name):
    if channel_last and func_name == "Unpooling":
        pytest.skip("Unpooling with channel_last is only supported in CUDA.")
    if channel_last and len(inshape) == len(kernel):
        pytest.skip(
