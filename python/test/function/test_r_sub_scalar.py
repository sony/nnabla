...
@pytest.mark.parametrize("seed", [315])
@pytest.mark.parametrize("val", [0.5, 1, 2])
def test_r_sub_scalar_forward_backward_with_reset(seed, val, ctx, func_name):
    from nbla_test_utils import function_tester
    rng = np.random.RandomState(seed)
    inputs = [rng.randn(2, 3, 4).astype(np.float32)]
    reset_inputs = [rng.randn(3, 2, 4).astype(np.float32)]
    function_tester(rng, F.r_sub_scalar, ref_r_sub_scalar,
                    inputs, func_args=[val], ctx=ctx, func_name=func_name, reset_inputs=reset_inputs)
