...
@pytest.mark.parametrize('model_class, up_to_list', [
    ('YoloV2', ['detection', 'convdetect', 'lastconv']),
    ])
@pytest.mark.parametrize('image_size_factor', [1, 2])
@pytest.mark.parametrize('_dataset_name', ['VOC', 'COCO'])
@pytest.mark.parametrize('batch_size', [1, 5])
@pytest.mark.parametrize('training', [False, True])
@pytest.mark.parametrize('seed', [1223])
def test_nnabla_models_object_detection(model_class, up_to_list, image_size_factor, batch_size, training, _dataset_name, seed):
    nn.clear_parameters()
