...
def _check_sample(dist_fn, scipy_fn, param_fn, ref_sample_fn):
    ...
    assert_allclose(sample.d.mean(), ref_sample.mean(), atol=3e-2, rtol=3e-2)
    assert_allclose(sample.d.std(), ref_sample.std(), atol=3e-2, rtol=3e-2)
...
def _check_prob(dist_fn,
    ...
    assert_allclose(prob.d, ref_prob, atol=3e-2, rtol=3e-2)
...
def _check_columns(dist_fn,
    ...
    for i, column in enumerate(CHECK_COLUMNS):
        ...
        assert_allclose(v.d, ref_v, atol=3e-2, rtol=3e-2)
