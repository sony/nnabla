...
class RefAMSBound(RefSolver):
    ...
    def _update_impl(self, key, p, g):
        ...
        _update_amsbound(p, g, self.m[key], self.v[key], self.v_hat[key], self.t[key],
                         self.alpha, self.beta1, self.beta2, self.eps, self.final_lr, self.gamma, self.bias_correction)


def _update_amsbound(p, g, m, v, v_hat, t, alpha, beta1, beta2, eps, final_lr, gamma, bias_correction):
    if bias_correction:
        alpha_t = alpha * \
