...
def random_crop(image, label, crop_h, crop_w):
    ...
    if h >= crop_h and w >= crop_w:
        ...
        offset_h = int(random.uniform(0, max_offset_h))#rand no between 0 to max_offset --> int
...
def random_scale(image,label):
    ...
    num_steps = int((max_scale_factor - min_scale_factor) / scale_factor_step_size +1)
    ...
    scaled_image = np.zeros((int(image.shape[0]*scale),int(image.shape[1]*scale),image.shape[2]), dtype=np.float32)
    scaled_label = np.zeros((int(label.shape[0]*scale),int(label.shape[1]*scale),label.shape[2]), dtype=np.int32)


    scaled_image = cv2.resize(image, (int(image.shape[1]*scale),int(image.shape[0]*scale)), interpolation=cv2.INTER_LINEAR)

    scaled_label = cv2.resize(label, (int(label.shape[1]*scale),int(label.shape[0]*scale)), interpolation=cv2.INTER_NEAREST)
...
def pad(image, label=None, desired_size=513):
    ...
    new_im = cv2.copyMakeBorder(image, 0, new_size[0]-old_size[0], 0, new_size[1]-old_size[1], cv2.BORDER_CONSTANT,value=color)
    if label is not None:
        label = cv2.copyMakeBorder(label, 0, new_size[0]-old_size[0], 0, new_size[1]-old_size[1], cv2.BORDER_CONSTANT,value=[255,255,255])
        if label.ndim==2 :
            label = np.expand_dims(label, axis=2)
