...
namespace nbla {
  ...
  void GlobalFunctionCallback::call_pre_hooks(const CgFunctionPtr &f) {
    for(auto &p: pre_hooks_) p.second(f);
  }

  void GlobalFunctionCallback::call_post_hooks(const CgFunctionPtr &f) {
    for(auto &p: post_hooks_) p.second(f);
  }

  void GlobalFunctionCallback::set_pre_hook(const string& key, const function_hook_type& cb) {
    auto it = std::find_if(pre_hooks_.begin(), pre_hooks_.end(),
                           [&](const KeyCallbackPair& elem) {return elem.first == key;});

    if (it != pre_hooks_.end()) {
      NBLA_LOG_WARN("[GlobalFunctionCallback::set_pre_hook()] key %s already exists.", key);
      return;
    }
  ...
  void GlobalFunctionCallback::set_post_hook(const string& key, const function_hook_type & cb) {
    auto it = std::find_if(post_hooks_.begin(), post_hooks_.end(),
                           [&](const KeyCallbackPair& elem) {return elem.first == key;});

    if (it != post_hooks_.end()) {
      NBLA_LOG_WARN("[GlobalFunctionCallback::set_post_hook()] key %s already exists.", key);
      return;
    }
  ...
  void GlobalFunctionCallback::unset_pre_hook(const string& key) {
    ...
    if (it == pre_hooks_.end()) {
      NBLA_LOG_WARN("[GlobalFunctionCallback::unset_pre_hook()] key %s does't exists.", key);
      return;
    }
  ...
  void GlobalFunctionCallback::unset_post_hook(const string& key) {
    ...
    if (it == post_hooks_.end()) {
      NBLA_LOG_WARN("[GlobalFunctionCallback::unset_post_hook()] key %s does't exists.", key);
      return;
    }
