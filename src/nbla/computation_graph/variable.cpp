...
class BackwardCallback {
  ...
  void clear_output_buffers(
    ...
    const vector<pair<bool, bool>> &output_clear_flags) {
    if (clear_buffer_) {      
      for (vector<CgVariablePtr>::size_type o = 0; o < outputs.size(); ++o) {
        if (outputs[o]->persistent()) {
          continue;
        }
        if (output_clear_flags[o].first) {
          outputs[o]->variable()->data()->array()->clear();
        }
        if (output_clear_flags[o].second) {
  ...
  pair<vector<bool>, vector<pair<bool, bool>>>
  query_outputs_flags(const CgFunctionPtr func) {
    ...
    for (vector<CgVariablePtr>::size_type i = 0; i < outputs.size(); i++) {
      ...
      if (first && clear_initial_grad_) { // first visit
        ...
        // inputs.
        output_clear_flags[i] = {!it->second, !it->second};
      }
      first_visit[i] = first;
    }
    ...
    for (vector<CgVariablePtr>::size_type i = 0; i < inputs.size(); i++) {
      if (f->inplace_data(i)) {
        output_clear_flags[f->inplace_data_with(i)].first = false;
      }
    }
...
public:
  ...
  void operator()(CgFunctionPtr f) {
    ...
    vector<pair<bool, bool>> output_clear_flags;
    ...
    clear_output_buffers(outputs, output_clear_flags);
...
void CgVariable::backward(
  ...
  // Initial gradient, "grad", can be cleared if it is just temporary buffer
