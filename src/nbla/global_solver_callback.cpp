...
namespace nbla {
  ...
  void GlobalSolverCallback::call_pre_hooks() {
    for(auto &p: pre_hooks_) p.second();
  }

  void GlobalSolverCallback::call_post_hooks() {
    for(auto &p: post_hooks_) p.second();
  }

  void GlobalSolverCallback::set_pre_hook(const string& key, const update_hook_type & cb) {
    auto it = std::find_if(pre_hooks_.begin(), pre_hooks_.end(),
                           [&](const KeyCallbackPair& elem) {return elem.first == key;});

    if (it != pre_hooks_.end()) {
      NBLA_LOG_WARN("[GlobalSolverCallback::set_pre_hook()] key %s already exists.", key);
      return;
    }
  ...
  void GlobalSolverCallback::set_post_hook(const string& key, const update_hook_type & cb) {
    auto it = std::find_if(post_hooks_.begin(), post_hooks_.end(),
                           [&](const KeyCallbackPair& elem) {return elem.first == key;});

    if (it != post_hooks_.end()) {
      NBLA_LOG_WARN("[GlobalSolverCallback::set_post_hook()] key %s already exists.", key);
      return;
    }
  ...
  void GlobalSolverCallback::unset_pre_hook(const string& key) {
    ...
    if (it == pre_hooks_.end()) {
      NBLA_LOG_WARN("[GlobalSolverCallback::unset_pre_hook()] key %s does't exists.", key);
      return;
    }
  ...
  void GlobalSolverCallback::unset_post_hook(const string& key) {
    ...
    if (it == post_hooks_.end()) {
      NBLA_LOG_WARN("[GlobalSolverCallback::unset_post_hook()] key %s does't exists.", key);
      return;
    }
