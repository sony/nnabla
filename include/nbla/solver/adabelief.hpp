...
NBLA_REGISTER_SOLVER_HEADER(AdaBelief, float, float, float, float, float, bool, bool,
    bool, bool);
...
*/
template <typename T>
class NBLA_API AdaBelief : public Solver {
public:
    AdaBelief(const Context& ctx, float alpha, float beta1, float beta2,
        float eps, float wd, bool amsgrad, bool weight_decouple, bool fixed_decay,
        bool rectify);
    virtual ~AdaBelief();
