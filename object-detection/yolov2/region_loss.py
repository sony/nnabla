...
def build_targets_numpy(pred_boxes, target, anchors, num_anchors,
    ...
    coord_mask, cls_mask, tx, ty, tw, th, tconf, tcls = [np.zeros((nB, nA, nH, nW), dtype=np.float32) for _ in range(8)]
    ...
    for b in range(nB):
        ...
        for t in range(50):
            ...
            cur_ious = np.maximum(cur_ious, bbox_ious_numpy(cur_pred_boxes, cur_gt_boxes, x1y1x2y2=False))
    ...
    if seen < 12800:
       if anchor_step == 4:
           tx = anchors.reshape((nA, anchor_step))[:,2].reshape((1,nA,1,1))
           for t_axis, t_n in zip([0, 2, 3], [nB, nH, nW]):
               tx = np.repeat(tx, t_n, axis=t_axis)
           ty = anchors.reshape((num_anchors, anchor_step))[:,2].reshape((1,nA,1,1))
           for t_axis, t_n in zip([0, 2, 3], [nB, nH, nW]):
...
def create_network(batchsize, imheight, imwidth, args):
    ...
    x, y, w, h, conf = [v.reshape((nB, nA, nH, nW)) for v in output_splitted[0:5]]
    ...
    tx, ty, tw, th, tconf, coord_mask, conf_mask_sq = [nn.Variable(v.shape) for v in [x, y, w, h, conf, x, conf]]
    ...
    loss_cls = args.class_scale * F.sum(cls_mask_bb * F.softmax_cross_entropy(cls + cls_ones - cls_mask, tcls))
...
def forward_nnabla(args, region_loss_seen, output_in, target, vars, tvars):
    ...
    grid_x = np.tile(np.tile(np.linspace(0, nW-1, nW), (nH,1)), (nB*nA, 1, 1)).reshape(nB*nA*nH*nW)
    grid_y = np.tile(np.tile(np.linspace(0, nH-1, nH), (nW,1)).transpose(), (nB*nA, 1, 1)).reshape(nB*nA*nH*nW)

    anchor_w = np.array(args.anchors).reshape((nA, anchor_step))[:,0:1]
    anchor_h = np.array(args.anchors).reshape((nA, anchor_step))[:,1:2]
    anchor_w = np.tile(np.tile(anchor_w, (nB, 1)), (1, 1, nH*nW)).reshape(nB*nA*nH*nW)
    anchor_h = np.tile(np.tile(anchor_h, (nB, 1)), (1, 1, nH*nW)).reshape(nB*nA*nH*nW)
